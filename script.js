...
const winSpdTwentyAPI =
  "http://webapi19sa-1.course.tamk.cloud/v1/weather/latest/wind_speed";
...
async function fetchTempTwenty(table) {
    try {
      ...
      tableHead.innerHTML = '<tr><th scope="col">#</th><th scope="col">Time</th><th scope="col">Temperature</th></tr>'
      tableBody.innerHTML = "";
      for (let i = 1; i < times.length; i++) {
        const t = times[i];
        const c = temperatures[i];
        const cellElement = document.createElement("tr");
        cellElement.innerHTML = '<td scope="row">' + i + '</td><td>'+ convertTime(t) +'</td><td>'+ c + ' °C' +'</td>'
  ...
  window.addEventListener('load', fetchTempTwenty(document.querySelector("table")))


async function fetchWinSpdTwenty() {
  try {
